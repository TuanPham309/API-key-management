...
if (isset($_POST['submit'])) {
    ...
    // Lấy thông tin từ form và kiểm tra tính hợp lệ của dữ liệu
    $version = $_POST['version'];
    $updateMessage = $_POST['update_message'];
    $fileName = $_POST['file_name'];
    $filePath = $_POST['file_path'];

    // Sử dụng câu lệnh truy vấn tham số hóa
    $sql = "INSERT INTO update_version (version, update_message, file_name, file_path) 
            VALUES (?, ?, ?, ?)";
    
    $stmt = $conn->prepare($sql);
    
    // Kiểm tra sự chuẩn bị thành công
    if ($stmt) {
        // Gán giá trị tham số
        $stmt->bind_param("ssss", $version, $updateMessage, $fileName, $filePath);

        // Thực hiện truy vấn
        if ($stmt->execute()) {
            echo "<script>
            alert('Thêm Version mới thành công!');
        ...
        echo "Lỗi trong quá trình chuẩn bị câu lệnh truy vấn: " . $conn->error;
    }
        ...
        <div class="card h-lg-100 overflow-hidden">
            <div class="card-header bg-light">
                <div class="row align-items-center">
                    <div class="col">
                        <h5 class="mb-0">Upload version </h5>
                    </div>
                </div>
            </div>
            <div class="card-body bg-light">
                ...
                <br>
                <form action="version_update.php" method="POST" accept-charset="utf-8" class="user">
               
              
                    <label for="version">Version:</label>
                    <input type="text" class="form-control" name="version" required><br>

                    <label for="update_message">Update Message:</label>
                    <textarea name="update_message" class="form-control" rows="4" required></textarea><br>

                    <label for="file">Select a file:</label>
                    <input type="file"  class="form-control"  name="file" id="file" required><br><br>
                    
                    <label for="file_name">File Name :</label>
                    <input type="text" class="form-control" name="file_name" id="file_name" value="auto_voice" required><br><br>

                    <label for="file_path">File Path :</label>
                    <input type="text" class="form-control" name="file_path" id="file_path" value="file_version" required><br><br>

                   
                    <button type="submit" class="btn btn-primary btn-rounded btn-block" name="submit" value="Upload"><i class="fas fa-save"></i>Upload</button>

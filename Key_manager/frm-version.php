            ...
            <table class="table table-sm table-dashboard fs--1 data-table border-bottom" data-options='{"responsive":false,"pagingType":"simple","lengthChange":true,"searching":true,"pageLength":10,"columnDefs":[{"targets":[0,6],"orderable":false}],"language":{"info":"_START_ to _END_ Items of _TOTAL_ — <a href=\"#!\" class=\"font-weight-semi-bold\"> Xem toàn bộ <span class=\"fas fa-angle-right\" data-fa-transform=\"down-1\"></span> </a>"},"buttons":["copy","excel"]}'>
                <thead class="bg-200 text-900">
                    <tr>
                        ...
                        <th class="sort pr-1 align-middle">version</th>
                        <th class="sort pr-1 align-middle">create_at</th>
                        <th class="sort pr-1 align-middle"> file_name</th>
                        <th class="sort pr-1 align-middle text-center">file_path</th>
                        <th class="sort pr-1 align-middle text-center">update_message</th>
                ...
                <tbody id="purchases">
                   ...
                   require_once "config.php";
                    ...
                    $query = "SELECT * FROM update_version";
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<input class="custom-control-input checkbox-bulk-select-target" type="checkbox" id="checkbox-' . $row["id"] . '" />';
                            echo '<label class="custom-control-label" for="checkbox-' . $row["id"] . '"></label>';
                            echo '</div>';
                            echo '</td>';
                            echo '<th class="align-middle"><a href="pages/customer-details.html">' . $row["version"] . '</a></th>';
                            echo '<td class="align-middle">' . $row["created_at"] . '</td>';
                            echo '<td class="align-middle">' . $row["file_name"] . '</td>';
                            echo '<td class="align-middle">' . $row["file_path"] . '</td>';
                               echo '<td class="align-middle text-center">' . $row["update_message"] . '</td>';
                             
                            echo '<td class="align-middle white-space-nowrap">';
                            echo '<div class="dropdown text-sans-serif">';
                            echo '<button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal mr-3" type="button" id="dropdown-' . $row["id"] . '" data-toggle="dropdown" data-boundary="html" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>';
                            echo '<div class="dropdown-menu dropdown-menu-right border py-0" aria-labelledby="dropdown-' . $row["id"] . '">';
                            echo '<div class="bg-white py-2">';
                            echo '<a class="dropdown-item" href="#!">Xem chi tiết</a>';
                            echo '<a class="dropdown-item" href="update-version.php?id=' . $row["id"] . '">Sửa</a>';

                            echo '<div class="dropdown-divider"></div>';
                            echo '<a class="dropdown-item text-danger" href="javascript:deleteKey(\'' . $row["id"] . '\')">Xóa</a>';
...
function deleteKey(id) {
    if (confirm("Bạn có chắc chắn muốn xóa bản cập nhật này?")) {
        $.ajax({
            type: "POST",
            url: "delete-version.php",
            data: { id: id },

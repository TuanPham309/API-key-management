                        ...
                        <li class="nav-item dropdown dropdown-on-hover"><a class="nav-link pr-0" id="navbarDropdownUser" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            ...
                            <div class="dropdown-menu dropdown-menu-right py-0" aria-labelledby="navbarDropdownUser">
                                <div class="bg-white rounded-soft py-2">
                                    ...
                                    <a class="dropdown-item" href="quanlytaikhoan.php">Quản lý tài khoản</a>
                                    <div class="dropdown-divider"></div>
...
if (isset($_GET['phone'])) {
    ...
    // Truy vấn để lấy thông tin người dùng dựa trên giá trị phone
    $query = "SELECT * FROM user_autotools WHERE phone = '$phone'";
    ...
    if ($result->num_rows > 0) {
        ...
        $formatted_expired = date('Y-m-d\TH:i', strtotime($current_user['expired']));
   
    } else {
        echo "Không tìm thấy người dùng có số điện thoại: $phone";
    }
    // Xử lý dữ liệu và lấy thông tin người dùng để sửa
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["phone"])) {
    ...
    $fpt = $_POST["fpt"];             // Thêm giá trị cho cột fpt
    $gg = $_POST["gg"];               // Thêm giá trị cho cột gg
    $ttsfree = $_POST["ttsfree"];     // Thêm giá trị cho cột ttsfree
    $voicemaker = $_POST["voicemaker"]; // Thêm giá trị cho cột voicemaker
    $trans = $_POST["trans"];         // Thêm giá trị cho cột trans

    // Câu truy vấn để cập nhật dữ liệu
    $query = "UPDATE user_autotools SET UserName = '$UserName', Pass = '$Pass', Machine_Id = '$Machine_Id', status = '$status', api_google = '$api_google' , expired = '$expired', fpt = '$fpt', gg = '$gg', ttsfree = '$ttsfree', voicemaker = '$voicemaker', trans = '$trans' WHERE phone = '$phone'";
    ...
    <div class="col-lg-12">
        <div class="card h-lg-100 overflow-hidden">
            ...
            <div class="card-body bg-light">
                ...
                <br>
                <form action="update.php" method="POST" accept-charset="utf-8" class="user">
                    <input type="text"  class="form-control" name="phone" value="<?php echo $phone; ?>">
                    <div class="mb-3">
                        <label for="UserName">UserName:</label>
                        <input type="text" class="form-control" name="UserName" id="UserName" value="<?php echo $current_user['UserName']; ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="Pass">Password:</label>
                        <input type="text" class="form-control" name="Pass" id="Pass" value="<?php echo $current_user['Pass']; ?>" placeholder="Password">
                    </div>
                    <div class="mb-3">
                        <label for="Machine_Id">Mã máy:</label>
                        <input type="text" class="form-control" name="Machine_Id" id="Machine_Id" value="<?php echo $current_user['Machine_Id']; ?>" placeholder="Nhập mã máy hoặc định danh" required>
                    </div>
                    <div class="mb-3">
                        <label for="api_google">Api Google:</label>
                        <input type="text" class="form-control" name="api_google" id="api_google"  value="<?php echo $current_user['api_google']; ?>" placeholder="Nhập api gg" required>
                    </div>


                    <div class="mb-3">
                        <label for="gg"> Google TTS:</label>
                        <input type="text" class="form-control" name="gg" id="gg"  value= "https://texttospeech.googleapis.com/v1/text" required>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" name="expired" value="<?php echo $formatted_expired; ?>"  required />
...
<footer>
    <div class="row no-gutters justify-content-between fs--1 mt-4 mb-3">
        <div class="col-12 col-sm-auto text-center">
            <p class="mb-0 text-600">Copyright Tuan Pham <span class="d-none d-sm-inline-block">| </span><br class="d-sm-none" /> 2021 &copy;</p>
        </div>
        <div class="col-12 col-sm-auto text-center">
